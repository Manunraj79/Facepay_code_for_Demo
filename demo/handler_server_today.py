...
def base64_to_img(img_list):
    img_read = []
    for each in img_list:
        img1_ = base64.b64decode(each)
        img1_ = np.array(ImageOps.exif_transpose(Image.open(BytesIO(img1_))))
        img1_ = cv2.cvtColor(img1_, cv2.COLOR_BGR2RGB)
        img_read.append(img1_)
    return img_read


def write_face_features(train_data, model_name='Facenet', enforce_detection=True, detector_backend='retinaface',
...
def validate_user(date, img1, threshold=0.85, PATH='./images', LOG='./log_history'):
    t_start = time.time()
    model_name = 'Facenet'
        ...
        for each in os.listdir('./face_weights/'):
            ...
            if os.path.exists(face_repr_file):
                ...
                with concurrent.futures.ProcessPoolExecutor() as executor:
                    futures = [
                        executor.submit(run_multiprocess, img1, img1_representation, representation_list[i], threshold)
    ...
    if not face_read_flag:
        return {"verified": False, "comparisons": 0, "time": str(timedelta(seconds=t_end - t_start)), "vdb_id": None,
                "distance": 1.0}
    ...
    if True in res_df['verified'].tolist():
        return {"verified": True, "comparisons": res_df.shape[0], "time": str(timedelta(seconds=t_end - t_start)),
                "vdb_id": min(list(res_df.loc[res_df['verified'] == True]['vdb_id'])),
                "distance": min(list(res_df.loc[res_df['verified'] == True]['score']))}
    else:
        return {"verified": False, "comparisons": res_df.shape[0], "time": str(timedelta(seconds=t_end - t_start)),
                "vdb_id": None, "distance": 1.0}
